...
<head>
    ...
    <script>
        function addLigne() {
            ...
            div.innerHTML = `
                <select name="lignes[${index}][id_client]" required class="p-2 border rounded">
        ...
        <form method="POST" action="{{ route('commandes.preview') }}" onsubmit="debugForm(event)">
            ...
            <div id="lignes-container" class="mb-4">
                ...
                <div class="ligne flex space-x-2 mb-2">
                    <select name="lignes[0][id_client]" required class="p-2 border rounded">
                        <option value="">Sélectionner un client</option>
                        @foreach($clients as $client)
                            <option value="{{ $gamme->id }}" {{ old('lignes.0.id_client') == $client->id ? 'selected' : '' }}>
                                {{ $client->nom }}
                            </option>
                        @endforeach
                    </select>
